...
from app.ollama import chat_stream, chat_sync
...
def _tools_to_dicts(tools) -> list[dict]:
    """Convert Tool pydantic models to dicts for the Ollama client."""
    result = []
    for t in tools:
        params = {}
...
async def _compress_messages(messages: list[dict]) -> list[dict]:
    ...
    summary_prompt = [
        ...
        {
            "role": "user",
            "content": json.dumps(first_half),
        },
    ]

    response = await chat_sync(summary_prompt)
...
async def generate(request: GenerateRequest, http_request: Request):
    ...
    selected_skill_names = await select_skills(messages)
    skill_content = get_skill_content(selected_skill_names)
    ...
    emit(
        "prompt_built",
        {
            ...
            "selected_skills": list(selected_skill_names),
    ...
    ollama_tools = _tools_to_dicts(request.tools) if request.tools else []
    ...
    async def event_generator():
        ...
        try:
            for iteration in range(MAX_ITERATIONS):
                ...
                async for chunk in chat_stream(
                    ...
                    if "prompt_eval_count" in chunk:
                        total_usage.prompt_tokens += chunk["prompt_eval_count"]
                    if "eval_count" in chunk:
                        total_usage.completion_tokens += chunk["eval_count"]

                    if msg.get("tool_calls"):
                        tool_call_data = msg["tool_calls"][0]
                        tc_name = tool_call_data["function"]["name"]
                        tc_params = tool_call_data["function"].get("arguments", {})

                        emit(
                            "tool_call",
                            {"iteration": iteration, "tool": tc_name, "params": tc_params},
                            trace_id,
                        )

                        yield {
                            "event": "tool_call",
                            "data": json.dumps({"tool": tc_name, "params": tc_params}),
                        }
            ...
            summary_text = ""
            async for chunk in chat_stream(conversation):
                msg = chunk.get("message", {})
                content = msg.get("content", "")
                if content:
...
async def compress(request: CompressRequest):
    ...
    return CompressResponse(
        messages=[Message(**m) for m in compressed],
    )
